 ...
 * El control del tamaño de la colección SOLO está en el método add y remove
 */
public class ÁrbolBOLigado<C extends Comparable <C>>
	...
	/**
	 * Método que elimina al nodo recibido de este árbol.
	 * @param hoja el nodo hoja que se va a eliminar de este árbol.
	 */
	/* No debe entrar la raíz de este árbol a este método. */
	public void eliminaHoja(NodoBinario<C> hoja) {
		if(hoja == this.raiz) throw new IllegalArgumentException("Aquí no debe entrar la raíz");

		/* No debería haber problema con este cast porque cualquier
		 * otro árbol tendrá o NodoBOLigados o NodosAVL o
		 * NodosRojinegros y éstos dos últimos heredan del primero. */
		NodoBOLigado<C> padre = (NodoBOLigado<C>)hoja.getPadre();
		if(padre.getHijoD() == hoja) {
			padre.setHD(null);
		}
    ...
    public boolean remove(C o) {
        ...
        NodoBOLigado<C> encontrado = (NodoBOLigado<C>)this.busca(o);

        //si este árbol no contenía al elemento *o*
        if(encontrado == null) return false;

        //Si es hoja
        if(encontrado.esHoja()) {
	        ...
	        this.eliminaHoja(encontrado);
	        this.tam--;
	        return true;
        }

        //Este caso es necesario porque no es necesrio modificar la
        //raíz del árbol.
        NodoBOLigado<C> aux;
        NodoBOLigado<C> temp;
        while(!encontrado.esHoja()) {
	        if(encontrado.getHijoI() != null) {
		        //pequenito
		        temp = (NodoBOLigado<C>)encontrado.getHijoI();
		        aux = (NodoBOLigado<C>)temp.grandote();
		        aux.intercambiaDato(encontrado);

		        encontrado = aux;
	        } else {
		        //grandote
		        temp = (NodoBOLigado<C>)encontrado.getHijoD();
		        aux = (NodoBOLigado<C>)temp.pequenito();
		        aux.intercambiaDato(encontrado);
		        encontrado = aux;
	        }
        }
